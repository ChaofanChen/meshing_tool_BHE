...
{
	cout << "****************************************" << endl;
	cout << "* BHE SETUP TOOL                       *" << endl;
	cout << "* ------------------------------------ *" << endl;
	cout << "* Author: Chaofan Chen, Philipp Hein   *" << endl;
	cout << "* Last revision: 21.09.2018            *" << endl;
	cout << "****************************************" << endl << endl;
...
bool WriteGMSHgeo(const string project_name, const geometry &geom, const vector<bhe> &BHEs, const vector<additional_point> &add_points)
{
	...
	if (geo_file.is_open())
	{
		...
		geo_file << "Line(5) = {" << a << ", " << b << "};" << endl;
		geo_file << "Line {5} In Surface {1};" << endl << endl;
...
bool ComputeBHEelements(const vector<bhe> &BHEs, const vector<node> &nodes, vector<bhe_element> &bhe_elements, const int n_mat_groups, int &n_elems)
{
	...
	vector<node> bhe_nodes_1, bhe_nodes_2, bhe_nodes_bottom;

	// Loop over nodes
	for (j = 0; j < n_nodes; j++)
	{
		// Check if node is on BHE and copy to BHE node list
		if (nodes[j].node_x == BHEs[0].bhe_x && nodes[j].node_y == BHEs[0].bhe_y &&
			nodes[j].node_z <= BHEs[0].bhe_top && nodes[j].node_z >= BHEs[0].bhe_bottom)
			bhe_nodes_1.push_back(nodes[j]);
		if (nodes[j].node_x == BHEs[0].bhe_x && nodes[j].node_y >= BHEs[0].bhe_y && nodes[j].node_y <= BHEs[1].bhe_y &&
			nodes[j].node_z == BHEs[0].bhe_bottom)
			bhe_nodes_bottom.push_back(nodes[j]);
		if (nodes[j].node_x == BHEs[1].bhe_x && nodes[j].node_y == BHEs[1].bhe_y &&
			nodes[j].node_z <= BHEs[1].bhe_top && nodes[j].node_z >= BHEs[1].bhe_bottom)
			bhe_nodes_2.push_back(nodes[j]);
	}

	int n_BHE_nodes_1 = bhe_nodes_1.size();
	int n_BHE_nodes_2 = bhe_nodes_2.size();
	int n_bhe_nodes_bottom = bhe_nodes_bottom.size();
	int n_BHE_elems = n_BHE_nodes_1 + n_BHE_nodes_2 + n_bhe_nodes_bottom - 1;

	// Loop over and create BHE1 elements
	for (j = 0; j < n_BHE_nodes_1 - 1; j++)
	{
		...
		this_bhe_element.start_node = bhe_nodes_1[j].node_number;
		this_bhe_element.end_node = bhe_nodes_1[j + 1].node_number;

		// Copy to BHE element list
		bhe_elements.push_back(this_bhe_element);
	}

	// Loop over and create BHE_bottom elements
	for (j = 0; j < n_bhe_nodes_bottom - 1; j++)
	{
		// Increase element counter
		cnt_elem++;
		...
		if (j == 0)
		{
			this_bhe_element.start_node = bhe_nodes_bottom[j].node_number;
			this_bhe_element.end_node = bhe_nodes_bottom[j + 2].node_number;
		}

		if (j == n_bhe_nodes_bottom - 2)
		{
			this_bhe_element.start_node = bhe_nodes_bottom[j + 1].node_number;
			this_bhe_element.end_node = bhe_nodes_bottom[1].node_number;
		}

		else if (j != 0 && j != n_bhe_nodes_bottom - 2)
		{
			this_bhe_element.start_node = bhe_nodes_bottom[j + 1].node_number;
			this_bhe_element.end_node = bhe_nodes_bottom[j + 2].node_number;
		}
	...
	// Loop over and create BHE2 elements
	for (j = n_BHE_nodes_2 - 2; j > -1; j--)
	{
		// Increase element counter
		cnt_elem++;
		...
		this_bhe_element.element_number = cnt_elem;
		this_bhe_element.material_group = cnt_mat_group;
		this_bhe_element.start_node = bhe_nodes_2[j + 1].node_number;
		this_bhe_element.end_node = bhe_nodes_2[j].node_number;

		// Copy to BHE element list
		bhe_elements.push_back(this_bhe_element);
	}
	cout << "Created " << n_BHE_elems << " elements on BHE " << n_BHE_elems << endl;
